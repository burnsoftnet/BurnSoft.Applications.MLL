...
namespace BurnSoft.Applications.MLL.UnitTests.Listings
{
    ...
    public class PowderInventoryTests
    {
        ...
        public void GetNameTest()
        {
            ...
            try
            {
                string value = PowderInventory.GetName(_databasePath, _existingId, out _errOut);
                if (_errOut.Length > 0) throw new Exception(_errOut);
        ...
        [TestMethod, TestCategory("Inventory Listings - Powder")]
        public void GetQtyPerPowderTest()
        {
            bool bAns = false;
            try
            {
                double value = PowderInventory.GetQtyPerPowder(_databasePath, _existingId, out _errOut);
                if (_errOut.Length > 0) throw new Exception(_errOut);
        ...
        [TestMethod, TestCategory("Inventory Listings - Powder")]
        public void GetPricePerPowderTest()
        {
            bool bAns = false;
            try
            {
                double value = PowderInventory.GetPricePerPowder(_databasePath, _existingId, out _errOut);
                if (_errOut.Length > 0) throw new Exception(_errOut);
                TestContext.WriteLine($"VALUE RETURNED {value}");
                bAns = (value > 0);
            }
